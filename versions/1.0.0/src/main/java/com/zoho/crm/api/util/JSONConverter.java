...
public class JSONConverter extends Converter
{
	...
	private JSONObject getModuleDetailFromUserSpecJSON(String module) throws IOException, SDKException
	{
		...
		moduleDetail = Utility.getJSONObject(moduleDetailFromUserSpecJSON, module);
		if (moduleDetail != null && moduleDetail.length() == 0)
		{
			try (FileWriter file = new FileWriter(recordFieldDetailsPath))
			{
				moduleDetailFromUserSpecJSON.remove(module.toLowerCase());
				file.write(moduleDetailFromUserSpecJSON.toString());
				file.flush();
			}
		}
		return moduleDetail;
